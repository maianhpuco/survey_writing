...
class outlineWriter():
    ...
    def draft_outline(self, topic, reference_num = 600, chunk_size = 30000, section_num = 6):
        ...
        # Step 6: Merge all rough outlines into one coherent high-level outline
        section_outline = self.merge_outlines(topic=topic, outlines=outlines)

        # Step 7: Generate detailed subsection-level outlines for each section in the merged outline
        subsection_outlines = self.generate_subsection_outlines(
            topic=topic, section_outline=section_outline, rag_num=50)

        # Step 8: Combine section + subsection outlines into a full structured outline
        merged_outline = self.process_outlines(section_outline, subsection_outlines)

        # Step 9: Refine and edit the final outline for clarity, coherence, and formatting
    ...
    def chunking(self, papers, titles, chunk_size = 14000):
        paper_chunks, title_chunks = [], []  # Initialize lists to hold the output chunks

        # Calculate the total number of tokens in all papers combined
        total_length = self.token_counter.num_tokens_from_list_string(papers)

        # Estimate number of chunks based on total tokens and chunk_size
        num_of_chunks = int(total_length / chunk_size) + 1

        # Calculate average token count per chunk to try and balance them
        avg_len = int(total_length / num_of_chunks) + 1

        split_points = []  # Indices where new chunks will start
        l = 0  # Running token count for current chunk

        # Loop through each paper to find good split points
        for j in range(len(papers)):
            l += self.token_counter.num_tokens_from_string(papers[j])  # Add current paper token count
            if l > avg_len:  # If chunk gets too big...
                l = 0  # Reset chunk token count
                split_points.append(j)  # Mark a split here
                continue

        start = 0  # Start index of the first chunk

        # Use split points to slice `papers` and `titles` into chunks
        for point in split_points:
            paper_chunks.append(papers[start:point])  # Add chunk of papers
            title_chunks.append(titles[start:point])  # Add corresponding titles
            start = point  # Update start to next split point

        # Append the remaining papers and titles after the last split
        paper_chunks.append(papers[start:])
        title_chunks.append(titles[start:])

        return paper_chunks, title_chunks  # Return list of paper/title chunks
